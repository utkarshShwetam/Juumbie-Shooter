...
public class Player : MonoBehaviour
{
    ...
    IEnumerator Shoot()
    {
        ...
        GameObject bullet = Instantiate(Resources.Load("Bullets", typeof(GameObject))) as GameObject;
        ...
        gun.GetComponent<Animation>().Play("AKM_FIRE");
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(spawnPoint.transform.position, spawnPoint.transform.forward, out hit, 100))
        {
            ...
            if (hit.collider.name.Contains("Zombie1"))
            {
                if (!isShooting)
                {
